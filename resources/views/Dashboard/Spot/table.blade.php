...
<tr>
    ...
    <td width="200">{{ $item->building->name}}</td>

    <td width="200">{{ $item->building->parking->name}}</td>
    ...
    <td>
        <div class="d-flex">
            <a href=@if ($page=='trash' ) "{{ route('Dashboard.spot.restore', $item->id) }}"
                @else "{{ route('Dashboard.spot.edit', $item->id) }}" @endif>
            ...
            <form
                action="@if ($page == 'trash') {{ route('Dashboard.spot.forcedelete', $item->id) }}@else{{ route('Dashboard.spot.destroy', $item->id) }} @endif"
                method="post">
                @csrf
                @method('delete')
