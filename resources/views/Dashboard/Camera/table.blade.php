...
<tr>
    ...
    <td width="200">{{ $item->gate->parking->name}}</td>
    ...
    <td>
        <div class="d-flex">
            <a href=@if ($page=='trash' ) "{{ route('Dashboard.camera.restore', $item->id) }}"
                @else "{{ route('Dashboard.camera.edit', $item->id) }}" @endif>
            ...
            <form
                action="@if ($page == 'trash') {{ route('Dashboard.camera.forcedelete', $item->id) }}@else{{ route('Dashboard.camera.destroy', $item->id) }} @endif"
                method="post">
                @csrf
                @method('delete')
