                            ...
                            <td>
                                <div class="d-flex">
                                    <a href=@if ($page=='trash'
                                        ) "{{ route('Dashboard.parking_work.restore', $item->id) }}"
                                        @else "{{ route('Dashboard.parking_work.edit', $item->id) }}" @endif>
                                    ...
                                    <form
                                        action="@if ($page == 'trash') {{ route('Dashboard.parking_work.forcedelete', $item->id) }}@else{{ route('Dashboard.parking_work.destroy', $item->id) }} @endif"
                                        method="post">
                                        @csrf
                                        @method('delete')
