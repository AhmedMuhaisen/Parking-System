...
<tr>
    ...
    <td>
        <div class="d-flex">
            <a href=@if ($page=='trash' ) "{{ route('Dashboard.unit.restore', $item->id) }}"
                @else "{{ route('Dashboard.unit.edit', $item->id) }}" @endif>
            ...
            <form
                action="@if ($page == 'trash') {{ route('Dashboard.unit.forcedelete', $item->id) }}@else{{ route('Dashboard.unit.destroy', $item->id) }} @endif"
                method="post">
                @csrf
                @method('delete')
