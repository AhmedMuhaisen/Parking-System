@forelse ($login_attempt as $item)
<tr>

    <td width="200">{{ $item->vehicle_number }}</td>

    <td width="200">{{ $item->gate->name}}</td>
    ...
    <td>
        <div class="d-flex">
            <a href=@if ($page=='trash' ) "{{ route('Dashboard.login_attempt.restore', $item->id) }}"
                @else "{{ route('Dashboard.login_attempt.edit', $item->id) }}" @endif>
            ...
            <form
                action="@if ($page == 'trash') {{ route('Dashboard.login_attempt.forcedelete', $item->id) }}@else{{ route('Dashboard.login_attempt.destroy', $item->id) }} @endif"
                method="post">
                @csrf
                @method('delete')
