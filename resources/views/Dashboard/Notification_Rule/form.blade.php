...
@section('title', 'notification_rule')
@section('content')
        ...
        <div class="card">
            <div class="card-body create-notification_rule">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    ...
                    <a class="role-back btn btn-primary w-20 h-75" href="{{ route('Dashboard.notification_rule.index') }}"> Rol
                        Back</a>
                </div>
...
<div class="mb-3">
        ...
        <select name="entity_type" class="form-select     @error('entity_type')
            ...
            @foreach(['vehicle', 'camera', 'section', 'gate'] as $type)
                <option value="{{ $type }}" {{ old('entity_type',$notification_rule->entity_type) == $type ? 'selected' : '' }}>{{ ucfirst($type) }}</option>
            @endforeach
    ...
    <div class="mb-3">
        ...
        <select name="event_type" class="form-select     @error('event_type')
            ...
            @foreach(['create', 'move', 'open', 'delete', 'edit'] as $event)
                <option value="{{ $event }}" {{ old('event_type',$notification_rule->event_type) == $event ? 'selected' : '' }}>{{ ucfirst($event) }}</option>
            @endforeach
   ...
   <div class="mb-3">
        ...
        <select name="target_audience" id="targetAudience" class="form-select     @error('target_audience')
            is-invalid
            @enderror" required>
            <option value="all" {{ old('target_audience',$notification_rule->target_audience) == 'all' ? 'selected' : '' }}>All Users</option>
            <option value="admin" {{ old('target_audience',$notification_rule->target_audience) == 'admin' ? 'selected' : '' }}>Admins Only</option>
            <option value="user" {{ old('target_audience',$notification_rule->target_audience) == 'user' ? 'selected' : '' }}>Specific User</option>
            <option value="phone" {{ old('target_audience',$notification_rule->target_audience) == 'phone' ? 'selected' : '' }}>Phone Number</option>
            <option value="email" {{ old('target_audience',$notification_rule->target_audience) == 'email' ? 'selected' : '' }}>Email Address</option>
    ...
    <div class="mb-3 d-none" id="userSelect">
        ...
        <select name="user_id" class="form-select     @error('user_id')
            ...
            @foreach(App\Models\User::all() as $user)
                <option value="{{ $user->id }}" {{old('user_id',$notification_rule->user_id) == $user->id ? 'selected' : '' }}>{{ $user->first_name .' '.$user->second_name }}</option>
            @endforeach
    ...
    <div class="mb-3 d-none" id="phoneInput">
        ...
        <input type="number" name="phone" value="{{ old('phone',$notification_rule->phone) }}" class="form-control     @error('phone')
            is-invalid
    ...
    <div class="mb-3 d-none" id="emailInput">
        ...
        <input type="email" name="email" value="{{ old('email',$notification_rule->email) }}" class="form-control     @error('email')
            is-invalid
...
<div class="mb-3">
    ...
    @foreach(['system', 'email', 'sms'] as $channel)
        <label class="pretty-checkbox">
            <input type="checkbox" name="channels[]" value="{{ $channel }}"
                {{ isset($notification_rule) && in_array($channel, json_decode($notification_rule->channels ?? '[]')) ? 'checked' : '' }}>
    ...
    @enderror
        ...
        <x-textarea name="message" title="message"  value="{{ $notification_rule->message ?? '' }}"/>
    ...
    @foreach(['created_by', 'created_at'] as $additional)
        <label class="pretty-checkbox">
            <input type="checkbox" name="additional[]" value="{{ $additional }}"
                {{ isset($notification_rule) && in_array($additional, json_decode($notification_rule->additional ?? '[]')) ? 'checked' : '' }}>
            <span class="checkmark"></span>
            {{ ucfirst($additional) }}
        </label>
    ...
    @foreach(['edit', 'delete', 'resend' ,'show'] as $action)
        <label class="pretty-checkbox">
            <input type="checkbox" name="actions[]" value="{{ $action }}"
                {{ isset($notification_rule) && in_array($action, json_decode($notification_rule->actions ?? '[]')) ? 'checked' : '' }}>
            <span class="checkmark"></span>
            {{ ucfirst($action) }}
        </label>
