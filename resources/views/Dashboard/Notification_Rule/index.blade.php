@section('title', 'Show notification_rules')
...
<main id="main" class="main position-absolute">
    ...
    <div class="pagetitle">
        <div class="d-flex justify-content-between align-items-center my-3">
            ...
            <a href="{{ route('Dashboard.notification_rule.create') }}" class="btn btn-primary" style="
">add
        ...
        <div class="card">
            <div class="card-body right-thead">
                <div class="d-flex justify-content-between align-items-center my-3">
                    ...
                    @if ($page == 'index')
                    @can('notification_rule.index')
                    <a href="{{ route('Dashboard.notification_rule.trash') }}" class="btn btn-outline-danger" style="
                                  ">
                    ...
                    @else
                    @can('notification_rule.index')
                    <a href="{{ route('Dashboard.notification_rule.index') }}" class="btn btn-outline-primary">
                ...
                <table class="table table-striped">
                    ...
                    <thead>
                        ...
                        <tr>
                            <td scope="col" colspan="1" width='200'>
                                <x-input type="text" value="" name="entity_type" id="entity_type" title="search" />
                            </td>
                    ...
                    <tbody id="tableContainer">
                        @include('Dashboard.Notification_Rule.table', ['notification_rule' => $notification_rules,
                        'page' => $page])
                    </tbody>
                </table>
...
$('#printExcelReport').on('click', function () {
    ...
    window.open("{{ route('Dashboard.notification_rule.exportExcel') }}?" + query, '_blank');
});
...
$('#printPdfReport').on('click', function () {
    let query = $.param(datavalue());
    // فتح رابط الطباعة مع تمرير الفلاتر
    window.open("{{ route('Dashboard.notification_rule.exportPDF') }}?" + query, '_blank');
});
    ...
    $('#searchBtn').on('click', function () {
        ...
        $.ajax({
            url: "{{ route('Dashboard.notification_rule.search') }}",
            type: "GET",
            data: data,
