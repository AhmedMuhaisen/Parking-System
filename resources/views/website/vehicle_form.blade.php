...
<div class="modal" id="editModalvehicle{{ $vehicle->id ?? 'A' }}">
    <div class="modal-content">
        ...
        <form id="editFormvehicle{{$vehicle->id ?? 'A'}}" action="{{ route($route, $vehicle->id ?? 'A') }}"
            method="post" >
            @csrf
            ...
            <div class="form-group w-100 text-center mb-0">
                <img src="{{ asset($vehicle->image) }}" class="profile-img mb-0" alt="">
            </div>
            @endif
            ...
            <div class="form-group">
                ...
                <input type="text" id="vehicle_number{{ $vehicle->id ?? 'A' }}" name="vehicle_number"
                    value="{{ $vehicle->vehicle_number ?? null }}">
            </div>
            <div class="form-group">
                <label>Vehicle Type</label>
                <x-select id="vehicle_type{{ $vehicle->id ?? 'A' }}" name='vehicle_type' title="Vehicle Type"
                    value="{{ $vehicle->vehicle_type->id ?? 'A' }}" :array="$vehicles_type">
                </x-select>
