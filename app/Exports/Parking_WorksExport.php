...
class Parking_WorksExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell,WithMapping,WithColumnWidths
{
    ...
    {
     $parking_works=ParkingWork::search($this->request);

    return $parking_works->get();
 ...
 public function map($parking_work): array
    {
        return [
            $parking_work->id,
            $parking_work->icon,
            $parking_work->title,
            $parking_work->content,
            $parking_work->step, // ← تنسيق التاريخ
        ];
    }
    ...
    {
        return ['ID', 'icon', 'title', 'content', 'step'];
    }
  ...
  $sheet->mergeCells('A4:E4');
                $sheet->setCellValue('A4', 'parking_works Report');
       ...
       // تمييز الصفوف الفردية والزوجية
            for ($row = 6; $row <= ParkingWork::count() + 5; $row++) {
