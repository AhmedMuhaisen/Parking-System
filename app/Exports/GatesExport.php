...
class GatesExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell,WithMapping,WithColumnWidths
{
 ...
 public function map($gate): array
    {
        return [
            $gate->id,
            $gate->name,
            ...
            $gate->open_method,
            $gate->status,
            $gate->created_at->format('Y-m-d H:i'), // ← تنسيق التاريخ
        ];
    }
    ...
    {
        return ['ID', 'Name',  'Address','Parking','Type','Open Method', 'Status', 'Created At'];
    }
  ...
  $sheet->mergeCells('A4:H4');
                $sheet->setCellValue('A4', 'gate Report');
                 $sheet->getStyle('A4:H4')->getFont()->setBold(true);
                // تنسيق العناوين والتوسيط
                $sheet->getStyle('A1:H4')->getAlignment()->setHorizontal('center');
                 $sheet->getStyle('A5:H5')->getFont()->setBold(true);
                $sheet->getStyle('A5:H5')->getFill()->setFillType('solid')->getStartColor()->setRGB('D9E1F2'); // أزرق فاتح
       // تمييز الصفوف الفردية والزوجية
            for ($row = 6; $row <= gate::count() + 5; $row++) {
