...
class Login_AttemptsExport implements FromCollection, WithHeadings, WithEvents, WithCustomStartCell, WithMapping, WithColumnWidths
{
    ...
    public function collection()
    {
        ...
        $login_attempts = Login_Attempt::search($this->request);

        return $login_attempts->get();
    }

    public function map($login_attempt): array
    {
        return [
            $login_attempt->id,
            $login_attempt->vehicle_number,
            $login_attempt->gate->name,
    ...
    {
        return ['ID', 'vehicle_number','Gate', 'Created_at'];
    }
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) use ($settings) {
                ...
                $sheet->setCellValue('A4', 'login_attempts Report');
                ...
                for ($row = 6; $row <= Login_Attempt::count() + 5; $row++) {
